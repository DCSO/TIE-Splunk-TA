...
def filter_iocs(iocs, filters, fp=sys.stdout):
    tmpl_filters = {"confidence": None, "max_severity": None}
    filter_cache = {}
    ...
    for ioc in iocs:
        try:
            ...
            logger.error(
                "bad value for max_confidence; was {}".format(ioc["max_confidence"])
            )
            continue
        ...
        try:
            ...
            logger.error(
                "bad value for max_severity; was {}".format(ioc["max_severity"])
            )
            continue
        ...
        try:
            if f["confidence"].in_range(max_confidence) and f["max_severity"].in_range(
                max_severity
            ):
